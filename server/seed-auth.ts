...
async function seedAuth() {
  try {
    ...
    const authSchemaSQL = readFileSync(
      join(__dirname, '../database/auth-schema.sql'),
      'utf8'
    );
    ...
    const userResult = await query(
      ...
      [masterEmail, passwordHash, 'Administrador Master']
    );
    ...
    const roleResult = await query(
      `SELECT id FROM financeiro.role WHERE nome = 'SUPER_ADMIN'`
    );
    ...
    await query(
      ...
      [masterUser.id, roleResult.rows[0].id]
    );
